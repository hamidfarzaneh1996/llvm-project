...
isl::union_map scheduleProjectOut(const isl::union_map &UMap, unsigned first,
  ...
  LLVM_DEBUG(dbgs() << "Finished "<< UMap.get_map_list().size() << "\n");
  for (isl::map Map : UMap.get_map_list()) {
...
isl::union_map tryInvertSequence(isl::union_map Schedule) {
  ...
  while (!ScatterSet.is_empty()) {
    ...
    SubSchedule = invertLoop(SubSchedule);
...
isl::union_map tryInvertSequence2(isl::union_map Schedule) {
  auto IslCtx = Schedule.get_ctx();
  auto ScatterSet = isl::set(Schedule.range());
  ...
  //   Counter = Counter.add(PartLen);
  // }
    NewSchedule = NewSchedule.unite(Schedule);
  LLVM_DEBUG(dbgs() << "Sequence-flatten result is:\n  " << NewSchedule
                    << "\n");
  return NewSchedule;
}
isl::union_map tryInvertLoopSequential(isl::union_map Schedule) {
  auto IslCtx = Schedule.get_ctx();
  auto ScatterSet = isl::set(Schedule.range());
  ...
  // while (!ScatterSet.is_empty()) {
    ...
    auto Extent = ThisSet;
    auto Min = Extent.dim_min(0);

    LLVM_DEBUG(dbgs() << "Min bound:\n  " << Min << "\n");
    auto Max = Extent.dim_max(0);
    LLVM_DEBUG(dbgs() << "Max bound:\n  " << Max << "\n");
  // } 
    // auto ScatterFirst = ThisFirst.add_dims(isl::dim::set, Dims - 1);
    // auto newMap = isl::map::from_range(ThisFirst);
    // auto SubSchedule = Schedule.intersect_range(ThisFirst);
    // return SubSchedule;
    auto SubSchedule = scheduleProjectOut(Schedule, 0, 1);
  //   SubSchedule = invertLoop(SubSchedule);

  //   auto SubDims = scheduleScatterDims(SubSchedule);
    auto FirstSubSchedule = scheduleProjectOut(SubSchedule, 1, 0);
    auto FirstScheduleAff = scheduleExtractDimAff(FirstSubSchedule, 0);
  //   auto RemainingSubSchedule = scheduleProjectOut(SubSchedule, 0, 1);
  ...
  //   }
    ...
    auto FirstSubScatterMap = isl::map::from_range(ThisFirst);
  ...
  //   // isl_set_universe(ParamSpace). Bug?
    ...
    auto ScheduleWithOffset = isl::union_map(FirstScheduleAffWithOffset);
                                  // .flat_range_product(RemainingSubSchedule);
    NewSchedule = Schedule.unite(ScheduleWithOffset);
...
///   { Stmt[i] -> [i * (u_X() - l_X() + 1), ...] }
isl::union_map tryInvertLoop(isl::union_map Schedule) {
  
  assert(scheduleScatterDims(Schedule) >= 2);

  auto Remaining = scheduleProjectOut(Schedule, 0, 1);
  auto SubSchedule = invertLoop(Remaining);
  auto SubDims = scheduleScatterDims(SubSchedule);
...
isl::set getOnlyOneSet(isl::union_map Schedule, isl::set &in_Set) {
  auto Result = isl::set::empty(Schedule.get_space());
  int i = 0 ;
  for (isl::set currSet: in_Set.get_basic_set_list()) {
    auto Outprojected = currSet.project_out(isl::dim::out, 0, 1);
    if(i==1){
      return Outprojected;

    }
    i++;
    // Result = Result.add_dims(Outprojected);
  }
  return Result;
}
isl::set polly::invertDomain(isl::union_map Schedule) {
    auto theMap = Schedule.range_map();
    LLVM_DEBUG(dbgs() << "The map :\n  " << theMap<< "\n");
    auto Extent = isl::set(Schedule.range());
    LLVM_DEBUG(dbgs() << "OLD Rangeextent :\n  " << Extent << "\n");
    auto domain = Schedule.domain();
    LLVM_DEBUG(dbgs() << "Domain \n " << domain<< "\n");

    auto negExtent = Extent.neg();
    LLVM_DEBUG(dbgs() << "neg Rangeextent :\n  " << negExtent << "\n");

    auto map = isl::map::from_range(Extent);
    LLVM_DEBUG(dbgs() << "Map :\n  " << map<< "\n");

    auto negMap = isl::map::from_range(negExtent);
    LLVM_DEBUG(dbgs() << "newMap :\n  " << negMap<< "\n");
 
    // auto Min = Extent.dim_min(0);
    // auto Min2 = newExtent.dim_min(0);
    // Extent = isl::set(newMap.range());
  //   auto Min = Extent.dim_min(0);
  //   auto Min2 = newExtent.dim_min(0);

  //   LLVM_DEBUG(dbgs() << "New extent :\n  " << newExtent << "\n");
  //   LLVM_DEBUG(dbgs() << "Min bound:\n  " << Min << "\n");
  //   auto MinVal = getConstant(Min, false, true);
  //   auto Max = Extent.dim_max(0);
  //   LLVM_DEBUG(dbgs() << "Max bound:\n  " << Max << "\n");

  //   // auto newSchedule = scheduleProjectOut(Schedule, 0, 1);
  //   auto FirstScheduleAff = scheduleExtractDimAff(Schedule, 0);
  //   auto ParamSpace = Schedule.get_space().params();
  //   auto NewSchedule = isl::union_map::empty(ParamSpace);
  //   // NewSchedule.from_domain_and_range(Schedule.domain(), newExtent);
  //   // NewSchedule.add_map(newMap);
  //   // NewSchedule.factor_range
  ...
  //   auto FirstAff = scheduleExtractDimAff(Schedule, 0);
  //   auto Offset = multiply(FirstAff, LenVal);
  //   // auto newOffset = Offset.neg();
  //   auto newOffset = isl::union_pw_aff::empty(Schedule.get_space());
  //   // Offset = Offset.neg();
  // // auto Index = FirstSubScheduleNormalized.add(Offset);
  //   // auto IndexMap = isl::union_map(Offset);
  //   auto IndexMap = isl::union_map::empty(Schedule.get_space());

  //   // getOnlyOneSet(Schedule, newExtent);
  //   LLVM_DEBUG(dbgs() << "New extent :\n  " << newExtent << "\n");
  //   LLVM_DEBUG(dbgs() << "New extent size :\n  " << newExtent.size() << "\n");
  //   LLVM_DEBUG(dbgs() << "Offset :\n  " << Offset << "\n");
  //   LLVM_DEBUG(dbgs() << "Offset :\n  " << Offset.domain() << "\n");
    // isl::union_pw_aff::
    // auto offset3 = newExtent.
    // LLVM_DEBUG(dbgs() << "Offset :\n  " << offset3 << "\n");
    // Schedule.dom 
    // return Schedule.from_domain_and_range(Schedule.domain(),newExtent);

  auto newExtent = Extent.neg();
  return newExtent;


}
isl::union_map polly::invertLoop(isl::union_map Schedule) {
  auto Dims = scheduleScatterDims(Schedule);
  LLVM_DEBUG(dbgs() << "Recursive schedule to process:\n  " << Schedule
                    << "\n");

  if (Dims == 0) {
    LLVM_DEBUG(dbgs() << "Dims == 0\n");
    return Schedule;
  }

  if (Dims == 1){
    LLVM_DEBUG(dbgs() << "Dims == 1\n");
    ...
    // auto Min = Extent.dim_min(0);
    // auto Min2 = newExtent.dim_min(0);
    auto newMap = isl::map::from_range(newExtent);
    Extent = isl::set(newMap.range());
    auto Min = Extent.dim_min(0);
    auto Min2 = newExtent.dim_min(0);
  ...
  // auto Index = FirstSubScheduleNormalized.add(Offset);
    ...
    // Schedule.dom 
    // return Schedule.from_domain_and_range(Schedule.domain(),newExtent);
    return Schedule.from_domain(Offset.domain());
    // return Schedule;
    // NewSchedule = NewSchedule.from_domain_and_range(Offset.domain(),newExtent);
    // NewSchedule = NewSchedule.from_domain(Offset.domain());
    // NewSchedule.from_range(newExtent);
    // return NewSchedule;
    // return scheduleExtractDimAff(NewSchedule,0);
    // return scheduleProjectOut(NewSchedule,0,1);
    // return scheduleProjectOut(NewSchedule,0,1);
    // return NewSchedule.from_domain_and_range(Offset.domain(),getOnlyOneSet(Schedule, Extent));

    // IndexMap = IndexMap.add_map(newMap);
    // auto Result = IndexMap.flat_range_product(Schedule);
    
    // LLVM_DEBUG(dbgs() << "Loop-flatten result is:\n  " << Result << "\n");
    // return Result;
    // return Schedule.reverse();
    // return tryInvertLoopSequential(Schedule); 

    // return tryInvertSequence2(Schedule);

    // auto ScheduleWithOffset = isl::union_map(FirstScheduleAffWithOffset)
    //                               .flat_range_product(RemainingSubSchedule);
    // NewSchedule = NewSchedule.unite(FirstScheduleAff);
    // auto MaxVal = getConstant(Max, true, false);
    // auto tempMaxVal = getConstant(Max, true, false);

    // auto newSchedule = scheduleExtractDimAff(Schedule, 0);
    // auto SingleUMap = isl::union_map::empty(Schedule.get_space());
    // for (isl::map Map : Schedule.get_map_list()) {

    //   isl::map SingleMap = Map.project_out(isl::dim::out, 0, 0);
    //   SingleMap.neg();
    ...
    // auto newMinVal = MaxVal.sub(MaxVal).sub(MaxVal);
    // auto newMaxVal = MinVal.sub(MinVal).sub(MinVal);

    // auto FirstAff = scheduleExtractDimAff(Schedule, 0);
    // Extent = isl::set(newSchedue.range());
    // Min = Extent.dim_min(0);
    // LLVM_DEBUG(dbgs() << "Min bound:\n  " << Min << "\n");
    // MinVal = getConstant(Min, false, true);
    // Max = Extent.dim_max(0);
    // LLVM_DEBUG(dbgs() << "Max bound:\n  " << Max << "\n");
    // MaxVal = getConstant(Max, true, false);
  ...
  if (!isVariableDim(Schedule)) {
    ...
    auto NewScheduleSequence = tryInvertSequence(Schedule);
  ...
  auto NewScheduleLoop = tryInvertLoop(Schedule);
  if (NewScheduleLoop)
